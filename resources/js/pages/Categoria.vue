...
// Declaración necesaria (asumiendo que estás usando la función route de Ziggy)

// --- INTERFACES Y DECLARACIONES REQUERIDAS ---
...
// Estado para la edición (Necesario para v-model y lógica de edición)
...
const listarCategoria = async () => {
    ...
    try {
        ...
        if (respuesta.data.success) {
            // Aseguramos que el estado se mantenga como número si la API lo devuelve así
...
const enviarFormulario = async () => {
    ...
    try {
        ...
        Swal.fire({
            title: 'Cancelado',
            text: 'Hubo un error al guardar o datos duplicados.',
            icon: 'error',
        });
        console.error(error);
    }
...
// --- FUNCIONES DE EDICIÓN (Las que corregimos y que usaste) ---

const abrirEditarModal = (item: CategoriaData) => {
    // Clona los datos y convierte el estado 1/0 a true/false para el checkbox
...
const actualizarCategoria = async () => {
    if (!categoriaAEditar.value) return;

    const estadoNumerico = categoriaAEditar.value.estado ? 1 : 0;
    const categoriaId = categoriaAEditar.value.id;

    try {
        const respuesta = await axios.put(`/categorias-data/${categoriaId}`, {
            nombre_categoria: categoriaAEditar.value.nombre_categoria,
        ...
        let errorMessage = 'Hubo un error al actualizar la categoría.';

        if (axios.isAxiosError(error) && error.response && error.response.status === 422) {
            errorMessage = error.response.data.errors?.nombre_categoria ? error.response.data.errors.nombre_categoria[0] : 'Error de validación: El nombre de la categoría podría estar duplicado.';
        } else {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col items-center justify-center">
            <div class="mt-4 flex flex-col items-center justify-center">
                <p class="text-3xl text-amber-600">🐼 Gestión Categoria 🎀</p>
                <p class="text-2xl">{{ minombre }}</p><br />
            ...
            <div class="dashboard-settings">
                <div class="report-section my-4 flex flex-col items-center justify-center">
                    ...
                    <br />
                    <a href="/categorias-exportar-pdf"
            ...
            <div>
                            ...
                            <tr v-for="item in categorias" :key="item.id"
                                ...
                                <td class="space-x-2 px-6 py-4 text-center">
                                    <a class="group relative inline-block" href="#">
                                        <span
                                            class="absolute inset-0 translate-x-1.5 translate-y-1.5 bg-sky-300 transition-transform group-hover:translate-x-0 group-hover:translate-y-0 group-hover:bg-blue-500"></span>
                                        <span
                                    ...
                                    <a class="group relative inline-block" href="#">
                                        <span
                                            class="absolute inset-0 flex translate-x-1.5 translate-y-1.5 bg-red-300 transition-transform group-hover:translate-x-0 group-hover:translate-y-0 group-hover:bg-red-500"></span>
                                        <span
                                            class="relative inline-block border-2 border-current px-8 py-3 text-sm font-bold tracking-widest text-black uppercase"
                ...
                <div class="fixed inset-0 z-50 grid place-content-center bg-black/50 p-4" role="dialog"
                    ...
                    <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg dark:bg-gray-900">
                        ...
                        <form @submit.prevent="enviarFormulario" class="grid gap-4 py-4">
                            <div class="mt-2 grid grid-cols-4 items-center gap-4">
                                <Label for="nombre_categoria" class="text-right">Nombre</Label>
                            ...
                            <div class="grid grid-cols-4 items-center gap-4">
                                ...
                                <div class="col-span-3 flex items-center gap-2">
                                    <input type="checkbox" id="status" v-model="formulario.estado"
                ...
                <div class="fixed inset-0 z-50 grid place-content-center bg-black/50 p-4" role="dialog"
                    aria-modal="true" aria-labelledby="editModalTitle" v-if="paEditarModal && categoriaAEditar">
                    <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg dark:bg-gray-900">
                        <h2 id="editModalTitle" class="text-xl font-bold text-gray-900 sm:text-2xl dark:text-white">
                            Editar Categoría (ID: {{ categoriaAEditar.id }})
                        </h2>

                        <form @submit.prevent="actualizarCategoria" class="grid gap-4 py-4">
                            <div class="mt-2 grid grid-cols-4 items-center gap-4">
                                <Label for="edit_nombre" class="text-right">Nombre</Label>
                                <div class="col-span-3">
                                    <Input type="text" id="edit_nombre" v-model="categoriaAEditar.nombre_categoria
                                        " required />
                                </div>
                            </div>
                            <label for="edit_descripcion">
                                ...
                                <textarea id="edit_descripcion" v-model="categoriaAEditar.descripcion"
                                    class="mt-0.5 w-full resize-none rounded shadow-sm sm:text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                                    rows="4"></textarea>
                            </label>
                            <div class="grid grid-cols-4 items-center gap-4">
                                <Label for="edit_status" class="text-right">Estado</Label>
                                <div class="col-span-3 flex items-center gap-2">
                                    <input type="checkbox" id="edit_status" v-model="categoriaAEditar.estado"
                                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500" />
                                    <span class="text-sm text-gray-600">{{
